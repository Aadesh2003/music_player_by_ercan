...
import 'package:music_player_by_ercan/artist_screen.dart';
import 'package:music_player_by_ercan/album_screen.dart';

class SongsScreen extends StatefulWidget {
  late List<SongInfo> _songs = <SongInfo>[];
...
class _SongsScreenState extends State<SongsScreen> with TickerProviderStateMixin, WidgetsBindingObserver {
  late List<SongInfo> _originalSongs = <SongInfo>[];
  late List<bool> _isPlaying = <bool>[], _originalIsPlaying = <bool>[];
  late List<AnimationController> _controllers = <AnimationController>[], _originalControllers = <AnimationController>[];
  ...
  late int _currentIndex = 0, _artistIndex = 0, _albumIndex = 0, _screenIndex = 0;
  ...
  void initState() {
    ...
    if (widget._songs.isEmpty == true) {
      ...
      _horizontalPageController = TabController(vsync: this, length: 3);
      _horizontalPageController.animation?.addListener(() {
        _currentScreen.value = (_horizontalPageController.animation?.value.round() == 1)
            ? 'artists'
      ...
      _player.onPlayerCompletion.listen((void event) {
  ...
  void _changeSong(int _index) {
    if (_index == 1) {
      _songPageController.nextPage(duration: const Duration(milliseconds: 600), curve: Curves.easeInOutCubic);
    } else {
      _songPageController.previousPage(duration: const Duration(milliseconds: 600), curve: Curves.easeInOutCubic);
    }
  }
  ...
  void _searchSong(String _search) {
    if (_search.isEmpty) {
      ...
      _scrollController.animateTo(0.0, duration: const Duration(milliseconds: 400), curve: Curves.fastOutSlowIn);
    } else {
      List<SongInfo> _searchSongs = <SongInfo>[];
      ...
      for (int i = 0; i < _originalSongs.length; i++) {
        if (_originalSongs[i].displayName.toLowerCase().contains(_search.toLowerCase()) == true) {
  ...
  void _handleTap(int _index, bool _shouldRestart, bool _shouldPlay) {
    ...
    if (_isPlaying[_index] == false) {
      ...
      if (_shouldPlay == true) {
        _player.play(widget._songs[_index].filePath, isLocal: true);
  ...
  void _goToAlbum(int _index, int _a) {
    ...
    if (_a == 1) {
      for (int i = 0; i < widget._songs.length; i++) {
        if (widget._albums[_index].title == widget._songs[i].album) {
          _indexx = i;
          break;
        }
      }
  ...
  void _goToArtist(int _index, int _a) {
    ...
    if (_a == 1) {
      for (int i = 0; i < widget._songs.length; i++) {
        if (widget._artists[_index].name == widget._songs[i].artist) {
          _indexx = i;
          break;
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: NestedScrollView(
        ...
        headerSliverBuilder: (BuildContext context, bool _arg) => <SliverAppBar>[
          SliverAppBar(
                      ...
                      : (value == 'AlbumPage')
                          ...
                          : (value == 'artists')
                              ? Row(
                                  ...
                                  children: const <Widget>[Text('Artists')],
                                )
                              : (value == 'albums')
                                  ? Row(
                                      key: const Key('Albums Screen'),
                                      children: const <Widget>[Text('Albums')],
                                    )
                                  : (_isSearching == true)
                                      ? TextField(
                          ...
                          builder: (BuildContext context, String value, Widget? child) {
                            return AnimatedSwitcher(
                              ...
                              transitionBuilder: (Widget child, Animation<double> animation) => SlideTransition(
                                position: animation.drive(Tween<Offset>(begin: const Offset(0.0, 1.0), end: Offset.zero)),
                                child: FadeTransition(opacity: animation.drive(Tween<double>(begin: 0.1, end: 1.0)), child: child),
                              ),
                              child: (value == 'AlbumPage')
                                  ? Align(
                                      ...
                                      child: WillPopScope(
                                        ...
                                        child: AlbumPage(widget._songs, _isPlaying, widget._songs[_albumIndex].album, _handleTap, _player, _showBottomSheet),
                                      ),
                                    )
                                  : (value == 'ArtistPage')
                                      ? Align(
                                          ...
                                          child: WillPopScope(
                                            ...
                                            child: ArtistPage(
                                                widget._songs, _isPlaying, widget._songs[_artistIndex].artist, _handleTap, _player, _showBottomSheet),
                                          ),
                                        )
                                      : Align(
                                          key: const Key('songs'),
                                          ...
                                          child: NotificationListener<OverscrollIndicatorNotification>(
                                            ...
                                            child: TabBarView(
                                              key: const PageStorageKey<String>('HORIZONTAL'),
                                              controller: _horizontalPageController,
                                              children: <Widget>[
                                                RawScrollbar(
                                                  key: const ValueKey<String>('songsscreen_1'),
                                                  ...
                                                  child: ListView.builder(
                                                      key: const PageStorageKey<String>('songsscreen'),
                                                        ...
                                                        return Padding(
                                                          ...
                                                          child: AnimatedContainer(
                                                            ...
                                                            child: ListTile(
                                                              ...
                                                              onTap: () => _handleTap(index, true, true),
                                                ...
                                                ArtistScreen(widget._artists, widget._albums, _goToArtist, _goToAlbum),
                                                AlbumScreen(widget._albums, _goToAlbum),
                                              ],
                                            ),
                                          ),
                                        ),
                    ...
                    Positioned.fill(
                      ...
                      child: Align(
                        ...
                        child: GestureDetector(
                          onTap: () {
                            _songPageController = PageController(initialPage: _currentIndex, keepPage: false);
                            if (_isSearching != false) {
                              setState(() {
                                _isSearching = false;
                              });
                            ...
                            Navigator.of(context).push(MyRoute<dynamic>(
                                builder: (_) =>
                                    NowPlaying(_songPageController, _originalSongs, _player, _handleTap, _deleteDialog, _changeSong, _goToArtist, _goToAlbum)));
                          },
                          onHorizontalDragEnd: (DragEndDetails details) {
                            if (details.primaryVelocity! > 0.0) {
                              if (_currentIndex == 0) {
                                _handleTap(widget._songs.length - 1, false, true);
                              } else {
                                _handleTap(_currentIndex - 1, false, true);
                              }
                            ...
                            builder: (BuildContext context, bool value, Widget? child) {
                              return AnimatedContainer(
                                ...
                                decoration: BoxDecoration(
                                  color: (value == false) ? Colors.grey : MyColors.accentColor,
                                            ...
                                            child: Column(
                                              ...
                                              children: <Widget>[
                                                AnimatedDefaultTextStyle(
                                                  ...
                                                  curve: Curves.easeInOutCubic,
                                                  style: TextStyle(
                                                    color: (value == false) ? MyColors.darkColor : Colors.white,
                                                    fontSize: 16.0,
                                                  ...
                                                  child: Text(
                                                    _originalSongs[_currentIndex].title,
                                                    maxLines: 1,
                                                    overflow: TextOverflow.ellipsis,
                                                ...
                                                AnimatedDefaultTextStyle(
                                                  ...
                                                  child: Text(
                                                    _originalSongs[_currentIndex].artist,
                                                    maxLines: 1,
                                                    overflow: TextOverflow.ellipsis,
                                                    textAlign: TextAlign.left,
                                                  ),
                                              ...
                                              child: Row(
                                                ...
                                                children: <Widget>[
                                                  ...
                                                  IconButton(
                                                    padding: EdgeInsets.zero,
                                                    visualDensity: const VisualDensity(horizontal: -4.0),
                                                    onPressed: () => _handleTap(_currentIndex, false, true),
                                                    icon: AnimatedIcon(
                                                      icon: AnimatedIcons.play_pause,
                                                      progress: _originalControllers[_currentIndex],
                                                      color: MyColors.darkColor,
                                                      size: 30.0,
                                                    ),
                                                  ),
                                            ...
                                            builder: (BuildContext context, AsyncSnapshot<Duration> snapshot) {
                                              return Theme(
                                                ...
                                                child: Slider(
                                                  ...
                                                  onChanged: (double a) {},
                                                  value: (snapshot.hasData) ? snapshot.data!.inSeconds.toDouble() : 0.0,
                                                  max: (int.parse(widget._songs[_currentIndex].duration) / 1000),
                                                ),
                                              );
                                            },
                                          ),
      ...
      bottomNavigationBar: TabBar(
        ...
        onTap: (int _index) {
          if (_currentScreen.value == 'ArtistPage' || _currentScreen.value == 'AlbumPage') {
            if (_screenIndex == _index) {
              ...
              _horizontalPageController.index = _horizontalPageController.previousIndex;
              _screenIndex = _index;
              WidgetsBinding.instance!.addPostFrameCallback((Duration timeStamp) {
        ...
        return Column(
          ...
          children: <Widget>[
            ...
            ListTile(
              ...
              subtitle: Text(
                widget._songs[_index].displayName.substring(0, widget._songs[_index].displayName.length - 4),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(color: Colors.white),
              ),
        ...
        return AlertDialog(
          ...
          content: RichText(
            text: TextSpan(
                ...
                children: <TextSpan>[
                  ...
                  TextSpan(
                      text: widget._songs[_index].displayName.substring(0, widget._songs[_index].displayName.length - 4),
          ...
          actions: <Widget>[
            TextButton(
              ...
              onPressed: () {
                _deleteSong(widget._songs[_index].filePath, _index);
                Navigator.of(context).pop();
              },
        ...
        return WillPopScope(
          onWillPop: () async {
            Timer(const Duration(milliseconds: 350), () {
              Navigator.of(context).pushReplacement(MyRoute<dynamic>(builder: (_) => SongsScreen(widget._songs, widget._artists, widget._albums)));
            });
            return false;
          },
          child: AlertDialog(
  ...
  void _clearThumbnails() {
    ValueNotifier<bool> _clearThumbnailDontShowAgain = ValueNotifier<bool>(false);
    showGeneralDialog(
      barrierColor: Colors.black.withOpacity(0.63),
      barrierLabel: '',
      useRootNavigator: true,
                    ...
                    children: <Widget>[
                      RichText(
                        text: const TextSpan(
                            ...
                            children: <TextSpan>[
                              TextSpan(text: 'If some thumbnails / album arts are missing, click '),
                              TextSpan(text: 'GO TO SETTINGS > Storage > Clear Data', style: TextStyle(color: MyColors.accentColor)),
                              TextSpan(
                      ...
                      Row(
                        children: <Widget>[
                          Checkbox(
                              value: value,
                              fillColor: MaterialStateColor.resolveWith((Set<MaterialState> states) => MyColors.accentColor),
                              visualDensity: const VisualDensity(horizontal: -4.0),
                              onChanged: (bool? _choice) {
                                _clearThumbnailDontShowAgain.value = !_clearThumbnailDontShowAgain.value;
                              }),
                          ...
                          InkWell(
                            ...
                            child: const Text("Don't Show Again", style: TextStyle(color: MyColors.accentColor)),
                          ),
                        ],
                      ),
                      const Text('You can access this dialog anytime by pressing the header on All Songs page.',
